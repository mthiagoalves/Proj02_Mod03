...
const findDigiServices = () => {
  console.log(digimons);
  return digimons;
};

const findDigiIdServices = (digiId) => {
  const digimon = digimons.find((digimons) => digimons.id === digiId);

  return digimon;
};

const digiCreateServices = (newDigimon) => {
  const newId = digimons.length + 1;
  newDigimon.id = newId;
...
const digiUpdateServices = (digiId, editDigimon) => {
  editDigimon['id'] = digiId;

  const digiIndex = digimons.findIndex((digimon) => digimon.id == digiId);
  digimons[digiIndex] = editDigimon;

  return editDigimon;
};

const degiDeleteServices = (digiId) => {
  const digiIndex = digimons.findIndex((digimon) => digimon.id == digiId);

  return digimons.splice(digiIndex, 1);
};
